#CosineSimilarity loss를 적용한 코드입니다.
#코드 중간중간 변경한 부분이 있어서, #변경한 부분 이라고 써두었어요. 참고해주시면 감사하겠습니다
...
class Model(pl.LightningModule):
    def __init__(self, model_name, lr):
        ...
        #변경한 부분
        self.loss_func = torch.nn.CosineSimilarity(dim=1)
    
    #변경한 부분
    #cosine similarity loss 계산
    def compute_loss(self, logits, labels):
        cosine_similarity = self.loss_func(logits, labels)
    ...
    def training_step(self, batch, batch_idx):
        x, y = batch
        logits = self(x)
        loss = self.compute_loss(logits, y.float()) #변경한 부분
...
if __name__ == '__main__':
    ...
    trainer = pl.Trainer(accelerator="gpu", 
                         devices=1, 
                         max_epochs=config["model_params"]["max_epoch"], 
                         log_every_n_steps=1,
                         auto_lr_find = True) #변경한 부분
